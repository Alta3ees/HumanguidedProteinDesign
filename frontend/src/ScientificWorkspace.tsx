...
function Mindmap({ roots, selectedId, onSelect }: { roots: DesignNode[]; selectedId: string | null; onSelect: (id: string) => void }) {
  ...
  if (roots.length === 0) return <div className="empty-panel">No designs yet. Use “Register design” to add the first node.</div>;
  return (
...
function EvidenceFiles({ slug, paths }: { slug: string; paths: string[] }) {
  return <div className="attached-files">{paths.map((path) => {
    ...
    return <a key={path} className="local-file-link" href={localFileUrl(slug, path)} target="_blank" rel="noreferrer"><span className="file-icon">↗</span><span><b>{name}</b><small>{path}</small></span></a>;
  })}</div>;
}
...
function EvidenceImporter({ slug, design, onUpdated }: { slug: string; design: DesignNode; onUpdated: (project: ProjectDetail) => void }) {
  ...
  return <section className="import-section"><div className="section-heading compact"><div><p className="eyebrow">Local data</p><h3>Attach evidence</h3></div></div>
    <form className="import-form" onSubmit={submit}><div className="form-row"><label>Type<select value={sourceType} onChange={(e) => setSourceType(e.target.value)}><option value="computational">Computational</option><option value="experimental">Experimental</option><option value="literature">Literature</option><option value="note">Note</option></select></label><label>Source <span className="optional-label">optional</span><input value={sourceName} onChange={(e) => setSourceName(e.target.value)} /></label></div>
      ...
      <input ref={inputRef} type="file" multiple onChange={(e) => setFiles(Array.from(e.target.files ?? []))} /><div className="upload-zone" onClick={() => inputRef.current?.click()}><b>{files.length ? `${files.length} file${files.length === 1 ? "" : "s"} selected` : "Browse local files"}</b><span>NMR, SEC, CD, SPR, PDFs, images, raw instrument files, model outputs…</span></div>
...
function RosettaSummary({ entry }: { entry: EvidenceEntry }) {
  ...
  return <article className="record-card rosetta-card"><div className="record-title"><strong>{entry.source_name}</strong><span>{String(data.mutation ?? "evaluation")}</span></div><div className="score-grid">{previous != null && <div><span>Parent score</span><b>{previous.toFixed(2)}</b></div>}{mutant != null && <div><span>Design score</span><b>{mutant.toFixed(2)}</b></div>}{delta != null && <div><span>ΔScore</span><b className={delta <= 0 ? "score-good" : "score-bad"}>{delta >= 0 ? "+" : ""}{delta.toFixed(2)} REU</b></div>}</div></article>;
}
...
function DesignDetail({ design, slug, onClose, onUpdated, onSelectNew }: { design: DesignNode; slug: string; onClose: () => void; onUpdated: (p: ProjectDetail) => void; onSelectNew: (id: string) => void }) {
  ...
  return <div className="detail-overlay"><header className="detail-topbar"><div><p className="eyebrow">Scientific design record</p><h1>{design.label}</h1><p className="detail-lineage">{design.lineage_label}</p></div><div className="detail-actions"><StatusBadge value={design.status} /><button className="secondary-button" onClick={() => setStructureDialog(true)}>Attach structure</button><button onClick={onClose}>← Back to design map</button></div></header>
    <main className="detail-page"><section className="detail-hero-grid"><div className="detail-card"><p className="eyebrow">Design identity</p><dl className="facts-grid large"><div><dt>Origin</dt><dd>{design.origin}</dd></div><div><dt>Decision</dt><dd>{design.decision?.outcome ?? "none"}</dd></div><div><dt>Created</dt><dd>{design.created_at.slice(0, 10)}</dd></div><div><dt>Structures</dt><dd>{design.structures.length}</dd></div></dl></div>
      ...
      {design.decision && <section className="detail-card decision-card-full"><div className="detail-card-header"><div><p className="eyebrow">Human guidance</p><h3>Decision record</h3></div><strong>{design.decision.outcome}</strong></div><div className="decision-grid"><div><span>Objective</span><p>{design.decision.objective || "—"}</p></div><div><span>Hypothesis</span><p>{design.decision.hypothesis || "—"}</p></div><div><span>Rationale</span><p>{design.decision.rationale || "—"}</p></div></div></section>}
      {computational.map((entry) => <RosettaDeepDive key={entry.id} entry={entry} />)}
      <section className="detail-card wide-section"><div className="detail-card-header"><div><p className="eyebrow">Structural hypotheses</p><h3>Structures</h3></div><button className="secondary-button" onClick={() => setStructureDialog(true)}>+ Attach structure</button></div>{design.structures.length === 0 ? <p className="muted">No first-class structure model attached.</p> : <div className="structure-grid">{design.structures.map((s) => <article className="record-card" key={s.id}><div className="record-title"><strong>{s.source}</strong><span>{s.method ?? ""}</span></div><p className="mono">{s.structure_path}</p><div className="metrics">{s.mean_plddt != null && <span>pLDDT {s.mean_plddt.toFixed(1)}</span>}{s.ptm != null && <span>pTM {s.ptm.toFixed(2)}</span>}{s.iptm != null && <span>ipTM {s.iptm.toFixed(2)}</span>}</div></article>)}</div>}</section>
...
export default function ScientificWorkspace() {
  ...
  return <main className="app-shell">
    <header className="topbar"><div><p className="eyebrow">Human-Guided Protein Design</p><h1>Research workspace</h1></div><div className="topbar-actions"><button className="primary-button compact-button" onClick={() => setNewProjectOpen(true)}>+ New project</button>{project && <button className="secondary-button" onClick={() => setRegisterOpen(true)}>+ Register design</button>}{selectedDesign && <button className="secondary-button" onClick={() => setStructureOpen(true)}>+ Attach structure</button>}<span className="local-pill">Local only</span><span className="version">v0.4 dev</span></div></header>
