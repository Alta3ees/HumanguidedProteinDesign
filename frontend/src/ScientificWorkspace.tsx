...
function DesignDetail({ design, slug, onClose, onUpdated, onSelectNew }: { design: DesignNode; slug: string; onClose: () => void; onUpdated: (p: ProjectDetail) => void; onSelectNew: (id: string) => void }) {
  ...
  const computational = design.evidence.filter((entry) => entry.source_type === "computational" && entry.data && Object.keys(entry.data).length > 0 && entry.data.analysis_type !== "position_saturation_scan");
